...
\section{Experience}
  \resumeSubHeadingListStart
     \resumeSubheading
      ...
      \resumeItem {Saved company \$1,242,700 annually in compute and data transfer costs by identifying an unoptimized, continuously running metrics collection process and helping replace it with an interval aggregation algorithm using Python.}
...
\section{Projects}
    \resumeSubHeadingListStart
      \resumeProjectHeading
          {\textbf{NBA Sportsbook Investment w/ Machine Learning} $|$ \emph{Python, XGBoost}}{}
          \resumeItemListStart
            \resumeItem{Generated 35.66\% return on portfolio per month with a 1.42 Sharpe Ratio by developing an NBA Sports Betting Strategy on Over/Under (OU) Lines when using a configuration of 2.5\% of fixed monthly bankroll per bet.}
            \resumeItem{Trained an XGBoost model to predict OU results with 56.37\% accuracy, gaining an expected 7.67\% ROI per bet.}
            ...
            \resumeItem{Enabled real-time trades monitoring by integrating DataDog with live scores and FanDuel's OU Line movements.}
          \resumeItemListEnd
      \resumeProjectHeading
