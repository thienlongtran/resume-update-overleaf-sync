...
\section{Projects}
    \resumeSubHeadingListStart
      \resumeProjectHeading
          {\textbf{NBA Sportsbetting Investment w/ Machine Learning} $|$ \emph{Python, XGBoost}}{}
          \resumeItemListStart
            \resumeItem{Generated 35.66\% return on investment (ROI) per month by developing an NBA Sports Betting Strategy on Over/Under (OU) Lines when using 2.5\% of fixed monthly bankroll per bet (14.26x bet size profit/month).}
            \resumeItem{Trained an XGBoost model to predict results of OU Lines with 56.37\% accuracy, providing a 7.67\% ROI per bet.}
            \resumeItem{Automated betting by utilizing CloudBet APIs to get latest data and place bets right before the start of a game.}
          \resumeItemListEnd
      \resumeProjectHeading
